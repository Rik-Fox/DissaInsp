...
class AngleGrinderEnvTests(unittest.TestCase):
    def setUp(self):
        ...
        self.graph_path = project_root / "graph.pkl"
        print(f"Using graph file at: {self.graph_path}")
        self.assertTrue(
            self.graph_path.exists(), f"Graph file not found at {self.graph_path}"
        )
    ...
    def test_initialization_and_reset(self):
        ...
        print(
            "  - checking that the real graph loads with a usable action space and observation vector"
        )
        ...
        print(
            "    -> this proves the environment can load the bundled angle grinder graph and expose its parts as observations"
        )
        ...
        print(
            "    -> this proves the root state starts with a valid action mask and no terminal flags"
        )
    ...
    def test_step_with_valid_action_transitions_to_a_new_state(self):
        ...
        print(
            "    -> this proves a valid action advances the environment to a new state and produces a meaningful reward"
        )
    ...
    def test_observation_payload_contains_state_and_time_details(self):
        ...
        self.log(
            "Observation payload",
            "Observation includes the expected state details and action time",
        )
        env = AngleGrinderEnv(graph_path=self.graph_path)
        observation, info = env.reset()

        print(
            "  - checking that the observation vector is backed by the current state metadata"
        )
        self.assertEqual(observation.shape, (len(env.part_list),))
        ...
        print(
            "    -> this proves the environment exposes the structural state information alongside the observation vector"
        )

        print(
            "  - checking that a valid action carries the expected time-related information"
        )
        valid_action = int(np.argmax(info["action_mask"]))
        next_observation, reward, terminated, truncated, next_info = env.step(
            valid_action
        ...
        print(
            "    -> this proves the environment reports the updated state and retains enough action metadata for downstream time-based reasoning"
        )
    ...
    def test_inspection_actions_record_outcomes_without_changing_physical_state(self):
        ...
        self.log(
            "Inspection action",
            "Inspection updates the state history and preserves the physical state",
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(
                "  - applying an inspection action and checking that it stays in the same state while recording the result"
            )
            ...
            print(
                "    -> this proves inspection actions record their outcome in the history and keep the physical state unchanged"
            )
    ...
    def test_print_actions_and_components_lists_the_graph_contents(self):
        ...
        self.log(
            "Graph summary",
            "The environment can print a readable action/component summary",
        )
        env = AngleGrinderEnv(graph_path=self.graph_path)
        ...
        print(
            "    -> this proves the environment can describe its action space and component list for debugging"
        )
    ...
    def test_step_with_invalid_action_ends_episode_with_penalty(self):
        ...
        print(
            "  - applying an out-of-range action to confirm invalid actions are rejected"
        )
        ...
        print(
            "    -> this proves invalid actions are rejected with a penalty and the environment remains in a terminal failed state"
        )
